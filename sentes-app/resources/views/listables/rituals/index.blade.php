<x-layoutDoom>
    <section class="w-75 flex-col items-center bg-light border-light">
        <h1 class="index-title special-elite-regular">Liste de rituels :</h1>
        ...
        @else
            ...
            @foreach ($ritualList as $ritual)
                <li>
                    <div class="flex-row">
                    <a href="{{ route('rituals.list.show', $ritual->id) }}" class="transparent-button special-elite-regular uppercase w-50">
                        Rituels "{{ $ritual->name }}" -
                    ...
                    <div class="w-25">
                        <a href="{{ route('rituals.list.edit', $ritual->id) }}" class="transparent-button special-elite-regular">
                            <i class="fa-solid fa-pen-to-square"></i>
                        </a>
                    </div>
                    <form action="{{ route('rituals.list.destroy', $ritual->id) }}" method="POST" class="w-25">
                        @csrf
                        @method('DELETE')
        ...
        <div class="w-75 show-button-container border-top-down-gradient">
            ...
            <a href="{{ route('rituals.list.create') }}" class="green-button special-elite-regular">Créer une nouvelle liste de rituels</a>
        </div>
    </section>
</x-layoutDoom>
