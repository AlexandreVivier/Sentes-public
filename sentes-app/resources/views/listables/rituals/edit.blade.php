<x-layoutDoom>
    <main class="event-frame-content bg-light flex-row justify-center border-light">
        ...
        <h1 class="index-title special-elite-regular">Modifier la liste de rituels {{$ritualList->name}} :</h1>
         <form action="{{ route('rituals.list.update', $ritualList->id) }}" method="POST" class="w-75 flex-col" enctype="multipart/form-data">
            @method('PATCH')
            @csrf
            <div class="form-input">
                <label for="name" class="text-shadowed">Nom de la liste de rituels :</label>
                <input type="text" name="name" id="name" class="w-100" value="{{$ritualList->name}}">
                @error('name')
                <p>
                    {{ $message }}
                </p>
            ...
            <div class="form-input">
                ...
                <textarea name="description" id="description" class="w-100">{{$ritualList->description}}</textarea>
                @error('description')
                <p>
                    {{ $message }}
                </p>
            ...
            <div class="w-100 show-button-container border-top-down-gradient">
                ...
                <button type="submit" class="green-button special-elite-regular">Modifier la liste {{$ritualList->name}}</button>
            </div>
        </form>
    </section>
</main>
